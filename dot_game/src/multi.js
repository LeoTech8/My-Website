...
// --- NETWORK TIMING ---
let lastSend = 0;
const SEND_RATE = 1000 / 20; // 20 updates/sec
...
function update() {
    ...
    // --- SMOOTH + PREDICT REMOTE PLAYER ---
    const SMOOTH = 0.25;

    if (isHost) {
        enemy.x += (targetEnemy.x - enemy.x) * SMOOTH;
        enemy.y += (targetEnemy.y - enemy.y) * SMOOTH;

        enemy.x += enemy.vx * 0.2;
        enemy.y += enemy.vy * 0.2;
    } else {
        player.x += (targetPlayer.x - player.x) * SMOOTH;
        player.y += (targetPlayer.y - player.y) * SMOOTH;
